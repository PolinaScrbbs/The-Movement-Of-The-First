...
class BaseApp(tk.Tk):
    def __init__(self):
        ...
        self.button1 = tk.Button(
            self.center_frame, text="Page 1", command=lambda: self.switch_page("Page 1")
        )
        self.button2 = tk.Button(
            self.center_frame, text="Page 2", command=lambda: self.switch_page("Page 2")
        )
        self.button3 = tk.Button(
            self.center_frame, text="Page 3", command=lambda: self.switch_page("Page 3")
        )
        ...
        self.user_name = tk.Label(
            self.right_frame, text="PolinaScrbbs", font=("Arial", 12), bg="lightgray"
        )
        self.user_name.pack(side=tk.LEFT, padx=5)

        self.profile_icon = ImageTk.PhotoImage(icon_image)  # Путь к картинке профиля
