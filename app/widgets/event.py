...
class EventApp:
    ...
    def create_events_tree(self, tab):
        ...
        self.event_tree = ttk.Treeview(tab, columns=("ID", "Event", "Type", "StartAt"), show="headings")
    ...
    def show_event_details(self, event_id):
        ...
        details_label = tk.Label(self.content_frame, text=f"Details for {event.title}", font=("Arial", 16, "bold"))
        details_label.pack(pady=20)

        description_label = tk.Label(self.content_frame, text=f"Description: {event.description}", font=("Arial", 12), anchor="w", justify="left")
        description_label.pack(pady=10)

        date_label = tk.Label(self.content_frame, text=f"Date: {event.start_at}", font=("Arial", 12), anchor="w", justify="left")
        date_label.pack(pady=10)

        back_button = tk.Button(self.content_frame, text="Back", command=self.show_events)
        back_button.pack(pady=20)

    def get_event_by_id(self, event_id):
        """Получаем событие по ID"""
        # Реализуйте логику поиска события по ID
        return next(event for event in get_events(get_session()) if event.id == event_id)
